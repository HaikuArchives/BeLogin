	...
	switch(Msg->what)
	{
		...
		case BL_ADD_ITEM:
		{
			...
			else
			{
				...
				Settings->GetBlocked()->AddItem(new BString(Signature));
		...
		case BL_REMOVE_ITEM:
		{
			if(ListView->CurrentSelection() >= 0)
			{
				...
				if(index == 1)
				{
					...
					BString strBlocked(((BStringItem*) ListView->ItemAt(ListView->CurrentSelection()))->Text());
					
					Settings->GetBlocked()->RemoveBlocked(strBlocked);
